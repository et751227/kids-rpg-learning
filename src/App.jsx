...
export default function RPGWordGameMain() {
      ...
      <div className="grid grid-cols-7 gap-4 max-w-xl mx-auto mb-4 px-4">
        {alphabet.map((char) => (
          <button
            ...
            className="bg-yellow-300 hover:bg-yellow-400 text-2xl font-bold py-4 px-4 rounded-xl shadow transition-transform active:scale-95 min-w-[56px] min-h-[56px]"
          >
            {char}
          </button>
        ))}
  ...
  if (loading || loadingData || !currentQuestion) {
    return (
      <div className="p-10 text-3xl text-center animate-bounce text-purple-700">
        🧠 載入拼字題中...
      </div>
    );
  }
  ...
  return (
    <div
      ...
      onClick={feedback ? loadNewQuestion : undefined}
    >
      ...
      <div className="text-lg text-gray-600 italic mb-2">題型：{currentQuestion.direction}</div>
      <div className="text-2xl font-bold text-blue-700 mb-2">請拼出：「{currentQuestion.questionText}」</div>
